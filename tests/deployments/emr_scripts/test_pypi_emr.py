...
class TestPyPiEMR:
    ...
    def test_run_job(self, _emr_config_mocked):
        ...
        input_dict = {'environment': 'dev',
                      ...
                      'github_repo': 'https://github.com/fabric8-analytics/f8a-pypi-insights'}
        response = pypi_emr.run_job(input_dict)
        status_code = response.get(
            'ResponseMetadata', {}).get('HTTPStatusCode')
        assert status_code == 200
        job_flow_id = response.get('JobFlowId')
        assert job_flow_id
        job_status = pypi_emr.aws_emr.get_status(job_flow_id)
